...
public class Pigeon {
    ...
    private Pigeon(int port) {        
        ...
        mGyro.getConfigurator().apply(Constants.SwerveConstants.pigeon2Config());
    }
    ...
    public Rotation2d getUnadjustedPitch() {
        return Rotation2d.fromDegrees(mGyro.getRoll().getValue().magnitude());
    }

    public Rotation2d getUnadjustedRoll() {
        return Rotation2d.fromDegrees(mGyro.getPitch().getValue().magnitude());
    }
}