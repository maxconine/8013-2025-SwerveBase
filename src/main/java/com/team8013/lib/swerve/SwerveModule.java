...
import com.team8013.frc2025.Ports;
import com.team8013.frc2025.subsystems.Subsystem;
...
public class SwerveModule extends Subsystem {
    ...
    public synchronized void readPeriodicInputs() {
        ...
        mPeriodicIO.velocity = Conversions.RPSToMPS(mDriveMotor.getRotorVelocity().getValue().magnitude(),
                Constants.SwerveConstants.wheelCircumference, Constants.SwerveConstants.driveGearRatio);

        mPeriodicIO.rotationPosition = Conversions.rotationsToDegrees(mAngleMotor.getRotorPosition().getValue().magnitude(),
                Constants.SwerveConstants.angleGearRatio);

        mPeriodicIO.drivePosition = Conversions.rotationsToMeters(mDriveMotor.getRotorPosition().getValue().magnitude(),
                Constants.SwerveConstants.wheelCircumference, Constants.SwerveConstants.driveGearRatio);;
    }
    ...
    public synchronized void writePeriodicOutputs() {
        ...
        if (mPeriodicIO.driveControlMode == ControlModeState.Velocity) {
            ...
            mDriveMotor.setControl(new DutyCycleOut(mPeriodicIO.driveDemand)); //(new DutyCycleOut(mPeriodicIO.driveDemand, true, false, false, false));
        }
    }
    ...
    public double getDriveMotorCurrent() {
        return mDriveMotor.getStatorCurrent().getValue().magnitude();
    }
